...
void TimeDisplay::displayTime()
{
    ...
    this->printValue(buffer, 0, 120, EPD_WIDTH, 120, (GFXfont *)&BIG_TIME_SCREEN_FONT, true, false);

    if (Peripherals::sht2x->idDataValid())
    {
        sprintf(buffer, "Temp: %0.2f C", Peripherals::sht2x->temperature);

        this->printValue(buffer, 0, 210, EPD_WIDTH / 2, 90, (GFXfont *)&SMALL_TIME_SCREEN_FONT, true, false);

        sprintf(buffer, "Humidity: %d %%RH", Peripherals::sht2x->humidity);

        this->printValue(buffer, EPD_WIDTH / 2, 210, EPD_WIDTH / 2, 90, (GFXfont *)&SMALL_TIME_SCREEN_FONT, true, false);
    }
    else
    {
        sprintf(buffer, "Temp: ---");
    ...
    if (Peripherals::iaq->hasValue() && Peripherals::iaq->isValid())
    {
        ...
        this->printValue(buffer, 0, 300, EPD_WIDTH / 2, 90, (GFXfont *)&SMALL_TIME_SCREEN_FONT, true, false);

        snprintf(buffer, 128, "TVOC: %i ppb", Peripherals::iaq->getTVOC());
        this->printValue(buffer, EPD_WIDTH / 2, 300, EPD_WIDTH / 2, 90, (GFXfont *)&SMALL_TIME_SCREEN_FONT, true, false);
    }
    else
    {
    ...
    this->printValue(buffer, 0, 390, EPD_WIDTH / 2, 90, (GFXfont *)&SMALL_TIME_SCREEN_FONT, true, false);

    snprintf(buffer, 128, "STK: %i", Status::thunderStrikes);
    this->printValue(buffer, EPD_WIDTH / 2, 390, EPD_WIDTH / 2, 90, (GFXfont *)&SMALL_TIME_SCREEN_FONT, true, false);

    snprintf(buffer, 128, "DST: %i km", Status::thunderDistance);
    this->printValue(buffer, 0, 480, EPD_WIDTH / 2, 90, (GFXfont *)&SMALL_TIME_SCREEN_FONT, true, false);
    ...
    this->printValue(buffer, EPD_WIDTH / 2, 480, EPD_WIDTH / 2, 90, (GFXfont *)&SMALL_TIME_SCREEN_FONT, true, false);
